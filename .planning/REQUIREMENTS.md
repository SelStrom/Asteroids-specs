...
- [ ] **AST-02**: Large астероид при уничтожении распадается на 2 Medium
- [ ] **AST-03**: Medium астероид при уничтожении распадается на 2 Small
- [ ] **AST-04**: Small астероид при уничтожении исчезает полностью
...
- [ ] **AST-08**: Новая волна начинается с 4 Large астероидами; каждая следующая +1 Large (макс. 12)
- [ ] **AST-09**: Астероиды первой волны появляются на безопасном расстоянии от корабля (минимум 20% ширины экрана)
- [x] **AST-10**: Object Pool для всех астероидов всех трёх размеров

### UFO (НЛО)

- [ ] **UFO-01**: Large UFO появляется случайно каждые 25–40 секунд с края экрана, движется прямолинейно с редкими сменами направления
- [ ] **UFO-02**: Small UFO появляется после счёта 10 000, стреляет в направлении корабля (точный)
- [ ] **UFO-03**: Large UFO стреляет в случайном направлении
- [ ] **UFO-04**: UFO проходит wrap-around экрана
- [ ] **UFO-05**: UFO исчезает после пересечения экрана (если не уничтожен)
- [ ] **UFO-06**: На экране одновременно не более одного UFO

### Scoring & Progression (Счёт и прогрессия)

- [ ] **PROG-01**: Очки начисляются: Large Asteroid 20, Medium 50, Small 100, Large UFO 200, Small UFO 1000
...
- [ ] **PROG-04**: Дополнительная жизнь выдаётся каждые 10 000 очков (максимум 6 жизней)
- [ ] **PROG-05**: При 0 жизнях — Game Over; игра прекращается, показывается экран Game Over
- [ ] **PROG-06**: Текущая волна отображается в HUD при старте каждой новой волны (кратковременный баннер)
...
- [ ] **VIS-01**: Все игровые объекты используют спрайты из предоставленного текстурного атласа
- [ ] **VIS-02**: Фон — чёрный, без текстур (аутентичный стиль Asteroids)
- [ ] **VIS-03**: Взрыв корабля: particle effect или анимация из спрайтов (минимум 4 фрейма)
- [ ] **VIS-04**: Взрывы астероидов: particle effect соответствующего размера
- [ ] **VIS-05**: HUD: счёт (верх-лево), жизни-иконки (верх-лево под счётом), High Score (верх-право)
- [ ] **VIS-06**: Main Menu: название игры, кнопка Play, кнопка Leaderboard
- [ ] **VIS-07**: Game Over экран: финальный счёт, кнопка «Submit Score», кнопка «Play Again», кнопка «Leaderboard»
- [ ] **VIS-08**: Leaderboard экран: Top-10 список с именем и очками, позиция текущего игрока, кнопка «Back»

### Audio (Аудио)

- [ ] **AUD-01**: Звук выстрела корабля (короткий щелчок/импульс)
- [ ] **AUD-02**: Звук тяги корабля (шипение, зациклен пока зажата кнопка тяги)
- [ ] **AUD-03**: Звук взрыва корабля (длинный взрыв)
- [ ] **AUD-04**: Звуки взрывов астероидов: отдельные для Large, Medium, Small
- [ ] **AUD-05**: Звук UFO (повторяющийся тон, пока UFO на экране)
- [ ] **AUD-06**: Фоновый пульс из двух чередующихся нот (бит); темп нарастает по мере уменьшения числа астероидов на экране
- [ ] **AUD-07**: Все звуки загружаются в AudioClip, воспроизводятся через AudioSource (объединены в пул или на GameManager)

### Leaderboards (Лидерборды)

- [ ] **LEAD-01**: При первом запуске (или если нет сохранённого токена) выполняется анонимная (Guest) аутентификация через UGS Authentication SDK
- [ ] **LEAD-02**: После Game Over, при нажатии «Submit Score», игрок вводит имя (max 16 символов) и счёт отправляется в UGS Leaderboard
- [ ] **LEAD-03**: Экран лидерборда отображает Top-10 записей глобального лидерборда (PlayerEntry: имя + очки)
- [ ] **LEAD-04**: Позиция текущего игрока отображается отдельно, даже если он не в Top-10
- [ ] **LEAD-05**: При недоступности сети (или ошибке UGS) показывается сообщение об ошибке; игра продолжается без краша
- [ ] **LEAD-06**: UGS Project ID и имя лидерборда задаются в ScriptableObject-конфигурации (не хардкод)
